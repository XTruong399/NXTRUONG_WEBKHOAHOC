...
<body>
    <header class="navbar">
        ...
        <nav>
          <ul class="menu">
          <a style="color: black;" href="{{ auth()->check() ? route('home_user') : route('trangchu') }}">Trang chủ</a>
            <li><a href="{{ route('category.products', ['id' => 3]) }}">Âm nhạc</a></li>
            <li><a href="{{ route('category.products', ['id' => 1]) }}">Kinh doanh</a></li>  <!-- Đường dẫn đến danh mục Kinh doanh -->
            <li><a href="{{ route('category.products', ['id' => 2]) }}">CNTT & Phần mềm</a></li>
        ...
        <button type="button" class="btn btn-default btn-md" onclick="window.location.href='{{ URL::to('/shopping-cart') }}'">
            <span class="glyphicon glyphicon-shopping-cart">
            </span>
            <b> Giỏ hàng </b>
        </button>
</form>
<form style="padding-top: 10px;" action="{{ route('my_courses') }}" method="GET">
...
<button class="btn btn-default btn-md" onclick="window.location.href='{{ URL::to('/register') }}'">Đăng ký</button>
<button type="button" class="btn btn-default btn-md" onclick="window.location.href='{{ URL::to('/shopping-cart') }}'">
            <span class="glyphicon glyphicon-shopping-cart">
            </span>
            <b> Giỏ hàng </b>
